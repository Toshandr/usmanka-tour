...
const BookingModal = ({ isOpen, onClose, tour, tariff }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Например, через fetch или axios к вашему API
      
      console.log('Заявка отправлена:', {
        tour: tour.shortTitle,
        tariff: tariff.name,
        price: tariff.price,
        ...formData
      });
      ...
      setMessage({
        type: 'error',
        text: 'Ошибка при отправке заявки. Попробуйте позже или свяжитесь с нами по телефону +7 (900) 123-45-67'
      });
    } finally {
      setLoading(false);
    }
